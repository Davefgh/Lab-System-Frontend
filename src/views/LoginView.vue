...
const shouldShowUsernameError = computed<boolean>(() => {
	return (
		(hasTriedSubmit.value && !!usernameErrorMessage.value) ||
		(!!username.value && !!usernameErrorMessage.value)
	);
});

const shouldShowPasswordError = computed<boolean>(() => {
	return (
		(hasTriedSubmit.value && !!passwordErrorMessage.value) ||
		(!!password.value && !!passwordErrorMessage.value)
	);
});
...
async function handleLogin() {
	...
	try {
		...
		// Redirect based on role if available, otherwise default to admin dashboard
		const role = response.data?.data?.role;
		if (role === "teacher") {
			// For now, redirect teachers to admin dashboard or a specific teacher view if it exists
			// Assuming teachers can access the admin dashboard for now
			...
			>
				...
				<div class="w-full space-y-4">
					...
					<div>
						...
						/>
						<p v-if="shouldShowUsernameError" id="username-error" class="mt-1 text-sm text-red-600">
					...
					<div>
						<div class="relative">
							...
							>
								...
								>
									<path
										d="M17.94 17.94A10.07 10.07 0 0112 20c-7 0-11-8-11-8a18.45 18.45 0 015.06-5.94M9.9 4.24A9.12 9.12 0 0112 4c7 0 11 8 11 8a18.5 18.5 0 01-2.94 4.06M1 1l22 22"
									/>
